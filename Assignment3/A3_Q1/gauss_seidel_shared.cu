            ...
            for(int tile_num = 0; tile_num < n; tile_num+=TILE_SIZE){
               ...
               index = (n+2)*(i+1)+j+1;
                ...
                for(int tile_off= 0; tile_off<TILE_SIZE; tile_off++){
                    ...
                    as[threadIdx.x + 1][tile_off + 1] = 0.2*(as[threadIdx.x+1][tile_off+1]+as[threadIdx.x+1][tile_off]+as[threadIdx.x][tile_off+1]+as[threadIdx.x+2][tile_off+1]+as[threadIdx.x+1][tile_off+2]);
		    A[index] = as[threadIdx.x + 1][tile_off + 1];
...
__global__ void gauss_seidel_kernel_general(float*A, int cols){
    ...
    while(!done){
        ...
        for (int i = col_start; i < col_end; i += TILE_SIZE) {
            ...
            for (int tile_off = 0; tile_off < TILE_SIZE; tile_off++) {
                temp = as[threadIdx.x+1][tile_off + 1];
                as[threadIdx.x + 1][tile_off + 1] = 0.2*(as[threadIdx.x+1][tile_off + 1]+as[threadIdx.x+1][tile_off]+as[threadIdx.x][tile_off+1]+as[threadIdx.x+1][tile_off+2]+as[threadIdx.x+2][tile_off+1]);
		A[index] = as[threadIdx.x+1][tile_off+1];
                local_diff += fabs(A[index]-temp);
                index++;
            }
