...
__global__ void mult_kernel(float*A, float*x, float*y, int span){
   ...
   __shared__ float as[THREADS_PER_BLOCK][TILE_SIZE];
   __shared__ float bs[TILE_SIZE];

   for(int row = id; row < n; row+=span){
       y[row] = 0.0;
       for(int tile_num=0; tile_num<n; tile_num+=TILE_SIZE){

           for(int tile_off=0; tile_off<TILE_SIZE; tile_off++){
               as[threadIdx.x][tile_off] = A[n*row+tile_num+tile_off];
               bs[tile_off] = x[tile_num+tile_off];
           }
           
           __syncthreads();

           for(int tile_off=0; tile_off<TILE_SIZE; tile_off++){
               y[row]+= as[threadIdx.x][tile_off]*bs[tile_off];
           }
           __syncthreads();
       }
   }
